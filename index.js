...
let herokuProxy = "https://cors-prox-server.herokuapp.com/";
...
function moreImagesInModal() {
  ...
  images.forEach((image) => {
    ...
    if (!image.title.includes(".svg")) imagesSearch(image.title, image_div);
  });
    ...
    success: function (json) {
      ...
      for (let i in pages) {
        ...
        if (pages[i].thumbnail) {
          imgDiv.append(
            `<img src="${pages[i].thumbnail.source}" class="square responsive-img">`
          );
...
function createActors(actors) {
  ...
  let length =
    actors.length > maxActorDisplay ? maxActorDisplay : actors.length;
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (let key in pages) {
        ...
        if (pages[key]) {
          image_div.append(
            `<div class="single-img"> <img src='${pages[key].imageinfo[0].url}' width="100%"> <div class='name'> ${name} </div> </div>`
          );
        }
      }
    },
